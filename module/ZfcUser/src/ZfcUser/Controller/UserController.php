...
class UserController extends AbstractActionController
{
	...
	protected $firstSocLogins;
	protected $iptelUserTable;
    ...
    public function loggedinAction(){
			// echo "loggedinAction";exit;
			$view = new ViewModel(array(
			));
			return $view->setTemplate("/zfc-user/user/logged-in.phtml");
	...
	public function indexAction()
    {
		...
		if( isset($providers) )
		{
			$adapter = $hybridAuth->authenticate($providers[0]);
			$user_contacts = $adapter->getUserProfile();
			$_SESSION['Zend_Auth']->photoURL=$user_contacts->photoURL;
			$_SESSION['Zend_Auth']->email=$user_contacts->email;
			$_SESSION['Zend_Auth']->displayName=$user_contacts->displayName;
			$view = new ViewModel(
				array(
					'baseUrl' 	=> $baseUrl,
					'basePath' 	=> $basePath,
				));
			return $view->setTemplate("/application/index/redirect-user.phtml");
		}
    }
